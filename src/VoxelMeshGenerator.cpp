...
HexMesh VoxelMeshGenerator::generateHexMesh(const VoxelGrid& voxelGrid) {
    ...
    if (occupiedVoxels.empty()) {
        return mesh; // Return empty mesh if no occupied voxels
    }
...
std::array<Point3D, 8> VoxelMeshGenerator::computeCornerNodes(const Voxel& voxel) {
...
std::vector<std::array<int, 8>> VoxelMeshGenerator::assignUniqueNodeIndices(
